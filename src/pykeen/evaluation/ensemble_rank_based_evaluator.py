...
from .ensemble_evaluator import EnsembleEvaluator, MetricResults, prepare_filter_triples
...
class EnsembleRankBasedMetricResults(MetricResults):
    ...
    @classmethod
    def create_random(cls, random_state: Optional[int] = None) -> "EnsembleRankBasedMetricResults":
...
class EnsembleRankBasedEvaluator(EnsembleEvaluator):
    ...
    # docstr-coverage: inherited
    def finalize(self) -> EnsembleRankBasedMetricResults:  # noqa: D102
        if self.num_entities is None:
            raise ValueError
        result = EnsembleRankBasedMetricResults.from_ranks(
    ...
    def finalize_multi(self, n_boot: int = 1_000, seed: int = 42) -> Mapping[str, Sequence[float]]:
        ...
        for i in range(n_boot):
            ...
            single_result = EnsembleRankBasedMetricResults.from_ranks(
...
class EnsembleSampledRankBasedEvaluator(EnsembleRankBasedEvaluator):
...
class EnsembleMacroRankBasedEvaluator(EnsembleRankBasedEvaluator):
