    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if batch_size is None and self.automatic_memory_optimization:
            ...
            model_sample = model[0] if isinstance(model, List) else model
            if model_sample.device.type == "cpu":
                logger.info(
...
def evaluate(
    ...
    *,
    ...
    # Send to device
    if device is not None and isinstance(model, List):
        model_list = []
        for m in model:
            m = m.to(device)
            model_list.append(m)
        model = model_list
    elif device is not None and isinstance(model, Model):
        model = model.to(device)
    elif device is None and isinstance(model, List):
